...
const SelectionItem: FC<SelectionItemProps> = ({
  ...
  useEffect(() => {
    backgroundColor.value = withTiming(isSelected ? themeColor : 'transparent', { duration: 300 });
  }, [isSelected]);
...
const AddScheduleFrequency: FC<AddScheduleFrequencyProps> = ({
  ...
  const toggleFrequency = (frequency: string) => {
    Keyboard.dismiss();
    setScheduleFrequency(frequency === scheduleFrequency ? null : (frequency as FrequencyType));
    if (frequency !== 'Weekly') {
      setSelectedDays([]);
    }
  };
