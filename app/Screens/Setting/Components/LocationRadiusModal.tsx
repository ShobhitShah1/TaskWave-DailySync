...
const LocationRadiusModal: FC<LocationRadiusModalProps> = ({
  ...
  // Map stays mounted after first open — avoids GL context teardown on each close
  const [hasBeenOpened, setHasBeenOpened] = useState(false);
  // Delay map render until modal animation finishes to avoid GL/animation thread conflict
  ...
  const handleModalShow = () => {
    ...
    // Wait for slide-in animation (400ms) to complete before rendering GL view
    setTimeout(() => setMapVisible(true), 420);
  };
  ...
  useEffect(() => {
    if (cameraRef.current && userLocation && mapVisible) {
  ...
  }, [selectedRadius, userLocation, zoomLevel, mapVisible]);

  const handleSelect = (radius: number) => setSelectedRadius(radius);
  ...
  return (
    <ReactNativeModal
      ...
      // Do NOT set hideModalContentWhileAnimating — it destroys the GL context each time
      ...
      hasBackdrop
      // useNativeDriver conflicts with MapLibre's GL thread — keep false
    ...
    >
      <View style={style.mainContainer}>
        ...
        <View style={style.mapContainer}>
          {hasBeenOpened && userLocation && !isLoading && (
            <MapView
              style={[style.map, { opacity: mapVisible ? 1 : 0 }]}
              ...
              {radiusCircle && (
                <ShapeSource id="radius-preview-source" shape={radiusCircle}>
                  <FillLayer
                    ...
                    style={{ fillColor: '#405DF0', fillOpacity: 0.2 }}
                  />
                  <LineLayer
                    id="radius-preview-border"
                    style={{ lineColor: '#405DF0', lineWidth: 3, lineOpacity: 0.8 }}
                  />
                </ShapeSource>
              )}
            </MapView>
